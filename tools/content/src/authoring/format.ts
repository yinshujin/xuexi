...
export type Step =
  /** lang "en": read by the English voice (English words and sentences in 英语 lessons). */
  | { say: string; lang?: 'en' }
  | { spotlight: string }
  | { laser: string }
  | { board: BoardItem[] }
...
- {"say":"Hello! I'm Danny.","lang":"en"}   // 英文内容用英语发音朗读：整句英文单独一个 say，加 "lang":"en"；
                                            // 中文讲解里不要夹英文单词（中文语音读不好），先说“跟我读”，再单独 say 英文
...
`,
  chinese: `### 硬性要求
- 字音、字形、词义必须准确：拼音带声调（如 xiǎo kē dǒu），和统编版课本一致；多音字、形近字要讲清楚。
- 课文只引用关键句（每页最多 2 句），不要整篇抄录课文；古诗可以完整出现。
- 只用这一册已经学过的知识；页面文字少、讲解靠 say；say 里只写汉字，不写拼音（拼音放在页面上给孩子看）。
- 讲解课 8–12 分钟（约 6–9 页、40–70 句 say）；技巧课 3–5 分钟（约 2–4 页、15–30 句 say）。
- 每节课都有一页“学法口诀”：2–4 句顺口好记的口诀（每句 5–9 字），和本课方法一致，老师先领读再逐句解释。
- 讲解课要有一页“识字写字”：本课 3–5 个重点生字的读音、结构、易错笔画，可用 table 或 cards；白板可以一步步写组词、扩词。
- 课堂小题按题型分成几个 quiz 场景：讲解课 3 组（判断题 3 道，选项固定 ["对","错"]；选择题 3–4 道，可含 1 道多选；
  填空与积累运用 3 道，题干用“（　）”表示空，做成选择题）；技巧课 2 组（判断题 2 道；选择与填空 2–3 道）。
- 所有算式（如果出现）也会被自动验算。
`,
  english: `### 硬性要求
- 英文拼写、大小写、标点和语法必须准确，和沪教牛津版（深圳）课本的单词、句型一致；只用本单元及以前学过的词句。
- 讲解用中文；英文单词和句子单独放一个 say 并加 "lang":"en"（例如先 {"say":"跟我读。"}，再 {"say":"Good morning!","lang":"en"}）。
  每个新单词至少领读两遍（可以写成一个 say："apple, apple"）。中文 say 里不要出现英文字母。
- 页面上英文写大一点、少一点：新单词用 cards（英文 + 中文意思），句型用 text 或 table。
- 讲解课 8–12 分钟（约 6–9 页、40–70 句 say）；技巧课 3–5 分钟（约 2–4 页、15–30 句 say）。
- 每节课都有一页“学法口诀”：中文口诀（2–4 句，每句 5–9 字）或简单的英文 chant，和本课内容一致，老师先领读再解释。
