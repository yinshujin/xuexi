...
| 二上 | 加减法解决问题 | 一共、还剩、求部分、比多比少、逆叙（“甲比乙多”求乙）、两步问题；列式选择 | 数的难度、逆叙和两步 | 见“多”就加、见“少”就减；忘记进位/退位 |
...
| 二上 | 测量 | 看刻度尺读长度（从 0 量、不从 0 量、断尺）、画线段、比较测量结果、非标准测量（拃、步、小棒） | 起点是否为 0、断尺 | 没有用末端减起点；次数和“尺子”长短的关系弄反 |
...
| 四上 | 线、垂直与平行 | 线段/射线/直线的判断和选择、端点个数、数线段、点到直线的距离、长方形中的平行与垂直、平行与垂直的推理 | 概念 → 判断 → 数线段 → 推理 | 线的特征弄混；平行与垂直的条件漏掉 |
