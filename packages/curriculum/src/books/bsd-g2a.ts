...
export const bsdG2a: Book = {
  ...
  sourceNote: [
    ...
    '目录已按家长拍摄的课本目录页核对：第一单元 100以内数加与减（二）p2；数学好玩 猜数游戏 p18；第二单元 测量（一）p20；第三单元 数一数与乘法 p30；',
    '第四单元 乘法口诀（一）p43；综合实践 画校园路线图 p53；第五单元 分一分与除法 p56；第六单元 图形的运动（一）p72；第七单元 乘法口诀（二）p77；',
    '第八单元 乘除法的应用（一）p87；综合实践 参加欢乐购物活动 p94；总复习 p98。',
  ...
  units: [
    ...
    unit(B, 1, '100以内数加与减（二）', [
      ...
      },
      {
        ...
        title: '数学好玩：猜数游戏',
        objectives: [
          '会根据“对”“不对”的回答确定这个数在什么范围',
          '知道从中间的数猜起，每次能排除大约一半的数',
          '体会有策略地猜，猜的次数可以尽可能少',
        ],
        keyPoints: [
          '重点：根据回答缩小范围，说清楚这个数最小、最大是几，还有几个数可能',
          '难点：边界——“比 50 大”不包括 50，“比 50 大吗？不对”说明这个数可能就是 50；从中间猜起',
          '常见错误：把边界算进或漏掉；数个数时两头的数没算上；一个一个地猜，次数太多',
        ],
        prerequisites: ['bsd-g2a.u1.compare-more-less'],
        localContexts: ['课间和同桌玩猜数游戏', '猜深圳地铁站的站台号'],
        lecture: {
          title: '猜数游戏：怎样猜得又快又准',
          minutes: 10,
          focus:
            '按教材“猜数游戏”展开：两人一组，一人在 1~100 中想一个数，一人提问，想数的人只能回答“对”或“不对”，猜的次数要尽可能少。先试着问“是 50 吗？”，发现一次只能排除一个数；“挑战升级”后改问“比 50 大吗？”，从中间的数猜起，每次都能排除大约一半的数，不断缩小范围。例题一：根据“比 50 大吗？对。比 75 小吗？不对。”说出这个数的范围；例题二：下一次问哪个数最好。最后介绍“你知道吗”：在 1~100 中任选一个数，用这种方法 7 次之内就能猜中。只用 100 以内的数，不讲“对数”等超纲概念。',
        },
        techniques: [
          {
            slug: 'middle-first',
            ...
            minutes: 4,
            focus:
              '教两步：①把每句回答翻译成范围，边界要想清楚（“比 50 大”从 51 开始，“比 50 大吗？不对”到 50 为止），在数轴或纸条上画出来；②从范围的中间问起（如 51~75 问“比 63 大吗？”），不管回答什么，都能排除大约一半。演示 2 个例子，对比把 50 算进“比 50 大”的错误，做 1 道小练习。',
            remedies: ['guess-range'],
          },
        ],
        practice: [
          {
            generatorId: 'g2.guess',
            minDifficulty: 1,
            maxDifficulty: 5,
          },
          {
            generatorId: 'g2.challenge',
            variant: 'guess#stretch',
            tier: 'stretch',
            minDifficulty: 2,
            maxDifficulty: 5,
          },
    ...
    unit(B, 3, '数一数与乘法', [
      {
        ...
        practice: [
          { generatorId: 'g2.mul.meaning', minDifficulty: 1, maxDifficulty: 2 },
          {
            generatorId: 'g2.challenge',
            variant: 'mul#stretch',
            tier: 'stretch',
      ...
      },
      {
        ...
        practice: [
          { generatorId: 'g2.mul.meaning', minDifficulty: 1, maxDifficulty: 3 },
          {
            generatorId: 'g2.challenge',
            variant: 'mul#stretch',
            tier: 'stretch',
      ...
      },
      {
        ...
        practice: [
          { generatorId: 'g2.mul.meaning', minDifficulty: 2, maxDifficulty: 5 },
          {
            generatorId: 'g2.challenge',
            variant: 'times#stretch',
            tier: 'stretch',
            minDifficulty: 2,
            maxDifficulty: 5,
          },
    ...
    unit(B, 4, '乘法口诀（一）', [
      ...
      },
      {
        ...
        title: '综合实践：画校园路线图',
        objectives: [
          '会用“向前走几步、向左（右）转”等说法描述一条路线',
          '会用步数估一估路有多长，用简单的图画出从教室到操场等地方的路线',
          '能看懂同学画的路线图，按图走一走',
        ],
        keyPoints: [
          '重点：用方向（前后左右）和步数描述路线，画出简单的路线图',
          '难点：转弯后左右会变；图上要标出起点、终点和几个明显的地方（如花坛、楼梯）',
          '常见问题：只画了路没写步数；转弯方向说反；图上没有起点和终点',
        ],
        prerequisites: ['bsd-g2a.u2.measure-informal'],
        localContexts: ['从教室走到操场、图书馆', '深圳学校里的升旗台和花坛'],
        lecture: {
          title: '画校园路线图',
          minutes: 10,
          focus:
            '综合实践课：先说一说从教室到操场要经过哪些地方，再用“向前走 20 步，向右转，再走 15 步”这样的话描述路线；用第二单元学过的步长估一估距离（步数多，路就长）。然后在纸上画出简单的路线图：标出起点、终点和途中的标志（花坛、楼梯、小卖部），线旁边写上步数。最后交换路线图，照着图走一走，看能不能走到。只用前、后、左、右和步数，不讲东南西北和比例尺。',
        },
        practice: [],
      },
    ]),
    ...
    unit(B, 5, '分一分与除法', [
      ...
      },
      {
        ...
        practice: [
          { generatorId: 'g2.div.table', minDifficulty: 1, maxDifficulty: 1 },
          {
            generatorId: 'g2.challenge',
            variant: 'div#stretch',
            tier: 'stretch',
    ...
    unit(B, 8, '乘除法的应用（一）', [
      ...
      },
      {
        ...
        objectives: [
          '会用乘法算买几个同样的东西要多少元',
          '会用除法算一些钱能买几个、每个多少元',
          '能在购物中选择合适的买法，算一算钱够不够、还剩多少',
        ],
        keyPoints: [
          '重点：在购物情境中选用乘法或除法（几个几用乘法，平均分、包含几个用除法）',
          '难点：两步问题（买两种东西一共多少元、带的钱还剩多少、钱够不够）',
          '常见错误：该乘却加（把“5 个 8 元”算成 5 + 8）；该除却乘',
        ],
        prerequisites: ['bsd-g2a.u8.farmyard-apply'],
        localContexts: ['学校跳蚤市场', '深圳的文具店、超市'],
        lecture: {
          title: '欢乐购物：乘除法帮我算钱',
          minutes: 10,
          focus:
            '综合实践课：以“欢乐购物”活动为情境，商品价格都是整元（不用角和分）。例题一：一个文具盒 8 元，买 4 个要多少元（4 × 8）；例题二：36 元正好买 4 个同样的玩具，每个几元（36 ÷ 4），或 36 元能买几个 9 元的玩具（36 ÷ 9）；例题三：带了 50 元，买 6 个 7 元的小汽车，还剩多少元，钱够不够。先说数量关系再列式，计算都在表内乘除法和 100 以内加减法范围里。',
        },
        practice: [
          {
            generatorId: 'g2.mul.table',
