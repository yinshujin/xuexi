...
const NOT_YET: Record<Subject, Record<Book['grade'], string>> = {
  ...
  writing: {
    2: '本册以后才学的生字和语法、修辞术语（如主谓宾、比喻拟人的名称）',
    4: '高年级的写作术语（如记叙文六要素之外的文体知识）和语法术语',
  },
};
...
const STRUCTURE: Record<Subject, { lecture: string; technique: string }> = {
  ...
  writing: {
    lecture:
      '结构：情境导入（一个孩子身边的写作难题）→ 讲一个写作方法（为什么、怎么做）→ 2 个例子在白板上对比“改前 / 改后”→ ' +
      '易错提醒 → 学法口诀（2~4 句顺口好记）→ 动笔小任务（写一两句话，给出参考写法）→ 一句话小结 → ' +
      '课堂小题按题型分组：判断题 3 道、选择题 3~4 道、句子练习 2~3 道（附答案讲评）。',
    technique:
      '结构：只讲一个写作小方法（如写完整、加上颜色和声音、用好标点、列提纲）→ 演示 2 个例子 → 对比一个常见错误说明错在哪 → ' +
      '学法口诀 → 小练习分两组：判断题 2 道、选择与句子练习 2~3 道（附答案）。',
  },
};
...
function rules(book: Book): string {
  ...
  switch (book.subject) {
    ...
    case 'writing':
      return (
        `要求：口语化短句，像老师面对面讲；每页文字不超过 40 字；例句和范文都是原创的、贴近${book.grade === 2 ? '二' : '四'}年级孩子的生活，` +
        `不整篇抄录课文或别人的作文；字词、标点准确，与${book.edition}教材的写话 / 习作要求一致；不要使用${notYet}；全部使用中文。`
      );
  }
}
