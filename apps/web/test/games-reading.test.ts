...
import { itemWeight, scoreExam, unitPapers } from '../src/lib/exams';
import {
  gameAnswerText,
  gamePrompt,
  isNumberQuestion,
...
describe('阅读题组 in the papers', () => {
  ...
  it('scores a passage by the questions answered right (earned out of its weight)', () => {
    ...
    expect(full.total).toBe(p.items.reduce((t, it) => t + itemWeight(it), 0));
    expect(itemWeight(p.items[at])).toBe(g.questions.length);
