...

{
  ...
  "title": "<课的标题>",
  "scenes": [ <场景>, ... ]          // 4–8 个场景，最后一个通常是 quiz
}
...
answer 是正确选项的序号（从 0 开始）；多个序号表示多选。每个 quiz 2–4 题，一个 quiz 一种题型，title 写题型名。

### 硬性要求
- 所有算式必须正确：脚本会自动验算形如 "a + b = c"、"a × b = c"、"a ÷ b = c" 的算式，算错会被拒绝。
- 只用这一册已经学过的知识；术语和北师大版课本一致。
- 页面文字少、讲解靠 say；say 里的数字和页面、白板上的一致。
- 讲解课 8–12 分钟（约 6–7 页、40–70 句 say）；技巧课 3–5 分钟（约 2–3 页、15–30 句 say）。
