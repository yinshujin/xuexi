...
describe('单元测试 papers', () => {
  ...
  it('scores out of 100 with a per-knowledge-point breakdown, and keeps the best', () => {
    ...
    // Points by weight (a reading passage is worth one point per question).
    const total = p.items.reduce((t, it) => t + itemWeight(it), 0);
    const got = p.items.reduce((t, it, i) => t + (i % 2 === 0 ? itemWeight(it) : 0), 0);
...
describe('连连看 / 拼一拼', () => {
  it('every 语文 and 英语 paper has one of each; math papers a 连连看 where the unit has sums', () => {
    for (const u of units) {
      for (const p of unitPapers(u.id)) {
        const kinds = p.items.flatMap((it) => (it.game && it.game.kind !== 'reading' ? [it.game.kind] : []));
